...
if (isset($_POST['btaddorgzer'])) {
  ...
  if (!isset($errMSG)) {
    ...
    if ($stmt->execute()) {
      ...
      $stmt = $session->runQuery("UPDATE organizer SET orgzerID=:orgzerID WHERE orgzerNo=:orgzerno");
      $stmt->bindParam(':orgzerno', $no);
      $stmt->bindParam(':orgzerID', $orgzerIDlist);
      if($stmt->execute()){
        $successMSG = "ทำการเพิ่มสำเร็จ";
        header("refresh:2; ../view/organizer_member.php");
      } else {
        $errMSG = "พบข้อผิดพลาด";
      }
    }
...
if (isset($_GET['delete_id'])) {
  ...
  if($stmt->execute()){
    ...
    header("Location: ../view/organizer_member.php");
	} else {
